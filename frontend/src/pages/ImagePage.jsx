    ...
    <div className="h-full overflow-y-auto scrollbar-thin">
              ...
              <div className="mt-4 grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Select value={style} onValueChange={setStyle}>
                    <SelectTrigger className="bg-vortex-elevated border-white/10 h-11" data-testid="image-style-select"><SelectValue /></SelectTrigger>
                    <SelectContent>
                ...
                <div>
                  ...
                  <Select value={aspect} onValueChange={setAspect}>
                    <SelectTrigger className="bg-vortex-elevated border-white/10 h-11" data-testid="image-aspect-select"><SelectValue /></SelectTrigger>
                    <SelectContent>
                ...
                <div className="flex items-center gap-2">
                  {[1, 2, 3, 4].map((n) => (
                    <button
                      key={n}
                      onClick={() => setCount(n)}
                      className={`flex-1 h-10 rounded-lg text-sm transition ${count === n ? "bg-vortex-cyan text-black font-semibold" : "bg-vortex-elevated border border-white/10 text-slate-300 hover:border-white/20"}`}
            ...
            <AnimatePresence mode="wait">
              {generating ? (
                <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={`grid gap-4 ${count > 1 ? "grid-cols-2" : "grid-cols-1"}`}>
                  {Array.from({ length: count }).map((_, i) => (
                    <div key={i} className={`${ASPECT_CLASS[aspect] || "aspect-square"} rounded-2xl glass relative overflow-hidden`} data-testid={`image-skeleton-${i}`}>
                      <div className="absolute inset-0 bg-gradient-to-br from-vortex-cyan/5 via-transparent to-vortex-purple/10 animate-pulse" />
                      <div className="absolute inset-0 flex items-center justify-center text-vortex-cyan">
                        <ImageIcon size={32} className="animate-vortex-pulse" />
                      </div>
                    </div>
                  ))}
                </motion.div>
                    ...
                    <div key={img.id || i} className="glass rounded-2xl overflow-hidden group" data-testid={`image-result-${i}`}>
                      <div className={`${ASPECT_CLASS[aspect] || "aspect-square"} bg-black relative`}>
                        ...
                        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition flex items-center justify-center opacity-0 group-hover:opacity-100">
                          ...
                          <Button size="icon" onClick={() => download(img)} className="btn-primary-vortex" data-testid={`image-download-${i}`}>
                ...
                <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass rounded-2xl p-16 text-center">
                  <ImageIcon size={48} className="mx-auto text-vortex-cyan opacity-60" />
      ...
      {lightbox && (
        <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-6" onClick={() => setLightbox(null)} data-testid="image-lightbox">
          <button onClick={() => setLightbox(null)} className="absolute top-6 right-6 text-white hover:text-vortex-cyan" data-testid="image-lightbox-close">
            <X size={28} />
          </button>
